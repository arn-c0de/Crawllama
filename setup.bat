...
if "%LLM_CHOICE%"=="" (
    ...
    echo %LLM_CHOICE% | findstr "1" >nul && python -c "import sys; f=open('requirements.txt','r',encoding='utf-8'); lines=f.readlines(); f.close(); installing=False; [[print(cleaned) for line in lines for cleaned in [line.strip()] if (line.startswith('# ===== LLM_OLLAMA') and not installing or (installing := line.startswith('# ===== LLM_OLLAMA'))) or (installing and line.startswith('# =====') and not (installing := False)) or (installing and cleaned and (cleaned := cleaned[1:].strip() if cleaned.startswith('#') else cleaned) and cleaned and not cleaned.startswith('====='))]]" >> requirements_temp.txt

    echo %LLM_CHOICE% | findstr "2" >nul && python -c "import sys; f=open('requirements.txt','r',encoding='utf-8'); lines=f.readlines(); f.close(); installing=False; [[print(cleaned) for line in lines for cleaned in [line.strip()] if (line.startswith('# ===== LLM_OPENAI') and not installing or (installing := line.startswith('# ===== LLM_OPENAI'))) or (installing and line.startswith('# =====') and not (installing := False)) or (installing and cleaned and (cleaned := cleaned[1:].strip() if cleaned.startswith('#') else cleaned) and cleaned and not cleaned.startswith('====='))]]" >> requirements_temp.txt

    echo %LLM_CHOICE% | findstr "3" >nul && python -c "import sys; f=open('requirements.txt','r',encoding='utf-8'); lines=f.readlines(); f.close(); installing=False; [[print(cleaned) for line in lines for cleaned in [line.strip()] if (line.startswith('# ===== LLM_ANTHROPIC') and not installing or (installing := line.startswith('# ===== LLM_ANTHROPIC'))) or (installing and line.startswith('# =====') and not (installing := False)) or (installing and cleaned and (cleaned := cleaned[1:].strip() if cleaned.startswith('#') else cleaned) and cleaned and not cleaned.startswith('====='))]]" >> requirements_temp.txt

    echo %LLM_CHOICE% | findstr "4" >nul && python -c "import sys; f=open('requirements.txt','r',encoding='utf-8'); lines=f.readlines(); f.close(); installing=False; [[print(cleaned) for line in lines for cleaned in [line.strip()] if (line.startswith('# ===== LLM_GROQ') and not installing or (installing := line.startswith('# ===== LLM_GROQ'))) or (installing and line.startswith('# =====') and not (installing := False)) or (installing and cleaned and (cleaned := cleaned[1:].strip() if cleaned.startswith('#') else cleaned) and cleaned and not cleaned.startswith('====='))]]" >> requirements_temp.txt
)
...
if /i "%INSTALL_API%"=="y" python -c "import sys; f=open('requirements.txt','r',encoding='utf-8'); lines=f.readlines(); f.close(); installing=False; [[print(cleaned) for line in lines for cleaned in [line.strip()] if (line.startswith('# ===== API') and not installing or (installing := line.startswith('# ===== API'))) or (installing and line.startswith('# =====') and not (installing := False)) or (installing and cleaned and (cleaned := cleaned[1:].strip() if cleaned.startswith('#') else cleaned) and cleaned and not cleaned.startswith('====='))]]" >> requirements_temp.txt

REM Install OSINT if selected
if /i "%INSTALL_OSINT%"=="y" python -c "import sys; f=open('requirements.txt','r',encoding='utf-8'); lines=f.readlines(); f.close(); installing=False; [[print(cleaned) for line in lines for cleaned in [line.strip()] if (line.startswith('# ===== OSINT') and not installing or (installing := line.startswith('# ===== OSINT'))) or (installing and line.startswith('# =====') and not (installing := False)) or (installing and cleaned and (cleaned := cleaned[1:].strip() if cleaned.startswith('#') else cleaned) and cleaned and not cleaned.startswith('====='))]]" >> requirements_temp.txt

REM Install Testing if selected
if /i "%INSTALL_TESTING%"=="y" python -c "import sys; f=open('requirements.txt','r',encoding='utf-8'); lines=f.readlines(); f.close(); installing=False; [[print(cleaned) for line in lines for cleaned in [line.strip()] if (line.startswith('# ===== TESTING') and not installing or (installing := line.startswith('# ===== TESTING'))) or (installing and line.startswith('# =====') and not (installing := False)) or (installing and cleaned and (cleaned := cleaned[1:].strip() if cleaned.startswith('#') else cleaned) and cleaned and not cleaned.startswith('====='))]]" >> requirements_temp.txt
